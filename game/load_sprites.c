...
/*   Updated: 2024/11/30 15:21:24 by carlos-j         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_sprites(t_game *game, int img_size)
{
	...
	game->sprites.win3 = mlx_xpm_file_to_image(game->minilibx.connect_mlx, WIN3,
			&img_size, &img_size);
	game->sprites.game_over1 = mlx_xpm_file_to_image(game->minilibx.connect_mlx,
			GAME_OVER1, &img_size, &img_size);
	game->sprites.game_over2 = mlx_xpm_file_to_image(game->minilibx.connect_mlx,
			GAME_OVER2, &img_size, &img_size);
	game->sprites.game_over3 = mlx_xpm_file_to_image(game->minilibx.connect_mlx,
			GAME_OVER3, &img_size, &img_size);
}
